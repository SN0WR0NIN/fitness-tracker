...
async function getChallengeSettingsUncached(): Promise<ChallengeSettings> {
  const rows = (await prisma.$queryRawUnsafe(
    'SELECT * FROM "ChallengeSetting" WHERE "id" = $1 LIMIT 1',
    "primary",
  ...
  if (rows[0])
    return {
      ...
      maintenanceMessage:
        rows[0].maintenanceMessage ||
        "New activity submissions are temporarily paused.",
      scoringRules: { ...DEFAULT_SCORING_RULES, ...rows[0].scoringRules },
    };
  await prisma.$executeRawUnsafe(
    'INSERT INTO "ChallengeSetting" ("id", "challengeName", "startDate", "endDate", "weeklyGoal", "scoringRules") VALUES ($1,$2,$3,$4,$5,$6::jsonb) ON CONFLICT DO NOTHING',
    "primary",
    "Kilo Golf Stay Active Challenge",
  ...
  return {
    ...
    maintenanceMessage:
      created.maintenanceMessage ||
      "New activity submissions are temporarily paused.",
    scoringRules: { ...DEFAULT_SCORING_RULES, ...created.scoringRules },
  };
}

export async function getAnnouncements(activeOnly = false) {
  return prisma.$queryRawUnsafe(
    `SELECT "id", "title", "message", "isActive", "createdAt", "updatedAt" FROM "Announcement" ${activeOnly ? 'WHERE "isActive" = true' : ""} ORDER BY "createdAt" DESC`,
  ) as Promise<ControlAnnouncement[]>;
}

export async function getAuditEntries(limit = 25) {
  return prisma.$queryRawUnsafe(
    'SELECT "id", "actorName", "action", "target", "details", "createdAt" FROM "AdminAudit" ORDER BY "createdAt" DESC LIMIT $1',
    limit,
  ) as Promise<AuditEntry[]>;
}

export async function getLatestScoreReviewNotes(): Promise<
  Map<string, string>
> {
  const rows = (await prisma.$queryRawUnsafe(`
    SELECT DISTINCT ON ("target") "target", COALESCE("details"->>'reviewNote', '') AS note
    FROM "AdminAudit"
    WHERE "details" ? 'reviewNote'
    ORDER BY "target", "createdAt" DESC
  `)) as Array<{ target: string; note: string }>;
  return new Map(rows.map((row) => [row.target, row.note]));
}
...
export async function getActiveColumnIds(): Promise<string[]> {
  const rows = (await prisma.$queryRawUnsafe(
    'SELECT "id" FROM "Column" WHERE "isActive"=true',
  )) as Array<{ id: string }>;
