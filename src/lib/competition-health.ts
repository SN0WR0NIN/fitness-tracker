...
type ColumnRow={id:string;name:string;_count:{members:number}};
...
export async function getCompetitionHealth():Promise<CompetitionHealth>{
  ...
  return{currentWeek,pending,rejectedThisWeek,flaggedPending,inactiveParticipants:participantRows.filter((user:ParticipantRow)=>!activeUsers.has(user.id)).length,nearTargetParticipants,participantCount:participantRows.length,finalizedWeeks:locks.filter(lock=>lock.status==='FINALIZED').length,completedWeeks,columns:columnRows.map((column:ColumnRow)=>{const active=activeByColumn.get(column.id)?.size??0;const members=column._count.members;return{id:column.id,name:column.name,members,active,participationPct:members?Math.round(active/members*100):0};}).sort((a:{participationPct:number;name:string},b:{participationPct:number;name:string})=>a.participationPct-b.participationPct||a.name.localeCompare(b.name)),backup:backup?{createdAt:backup.createdAt,activities:backup.counts.activities??0}:null};
}
