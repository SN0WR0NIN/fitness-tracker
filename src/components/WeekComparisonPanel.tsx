...
export default function WeekComparisonPanel({activities,embedded=false}:{activities:MetricActivity[];embedded?:boolean}){
  ...
  const content=<section className="rounded-2xl border border-white/10 bg-slate-900/60 p-5 sm:p-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[0.68rem] font-black uppercase tracking-[0.16em] text-sky-300">Week comparison</p><h2 className="mt-1 text-lg font-black">Compare your weeks</h2><p className="mt-1 text-xs text-slate-400">Approved activities only · compare training volume and performance.</p></div><div className="flex items-center gap-2"><WeekSelect value={a} weeks={weeks} onChange={setWeekA}/><ArrowRight className="h-4 w-4 text-slate-600"/><WeekSelect value={b} weeks={weeks} onChange={setWeekB}/></div></div>{weeks.length?<div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{cards.map(card=><CompareCard key={card.label} {...card}/>)}</div>:<p className="mt-5 text-sm text-slate-500">Complete an approved activity to start comparing weeks.</p>}</section>;\n  return embedded?content:<details className="dashboard-fold"><summary>Week comparison</summary>{content}</details>;
}
