...
import { normalizeRunSegments, type ScoringRules } from '@/lib/scoring';
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
        ...
        <section className="mt-5 space-y-4" aria-busy={actioningId !== null}>
          {filteredActivities.length ? filteredActivities.map((activity) => {
            ...
            const intervalSegments=normalizeRunSegments(activity.runSegments);
                ...
                <div className="p-5 sm:p-6">
                  ...
                  {editingId === activity.id ? <EditPanel activity={activity} users={users} editForm={editForm} setEditForm={setEditForm} save={() => saveEdit(activity)} cancel={() => setEditingId(null)} saving={mutationDisabled} /> : (
                    <div className="mt-5 grid gap-3 text-sm sm:grid-cols-2 xl:grid-cols-4">
                      <Detail label="Activity" value={intervalSegments.length?`${categoryLabels[activity.category]} · Interval`:categoryLabels[activity.category]} />
                  ...
                  {intervalSegments.length&&editingId!==activity.id?<div className="mt-4 rounded-xl border border-orange-300/20 bg-orange-300/[0.06] p-3"><p className="text-xs font-black uppercase tracking-wide text-orange-200">Interval pace groups</p><div className="mt-2 grid gap-2 sm:grid-cols-2">{intervalSegments.map((segment,index)=><div key={`${segment.kind}-${index}`} className="rounded-lg bg-black/15 px-3 py-2 text-xs text-slate-300"><strong>{segment.kind==='WORK'?'Work':'Recovery'} {index+1}</strong> · {segment.distance.toFixed(2)} km · {formatPace(segment.pace)}/km</div>)}</div></div>:null}
