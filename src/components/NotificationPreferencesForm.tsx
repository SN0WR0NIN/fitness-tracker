...
export default function NotificationPreferencesForm({ initial }: { initial: NotificationPreferences }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setValue(result);
      setMessage('Notification preferences saved for your account across devices.');
      window.dispatchEvent(new Event('kg:notifications-changed'));
      router.refresh();
    } catch (error) {
  ...
  return (
    <form onSubmit={save} className="space-y-5 rounded-2xl border border-white/10 bg-white/[0.04] p-5 sm:p-7">
      <fieldset disabled={busy} className="divide-y divide-white/10">
        {options.map((option) => (
          <label key={option.key} className="flex min-h-20 cursor-pointer items-center justify-between gap-5 py-4">
            <span>
            ...
            <input
              ...
              onChange={(event) => {
                ...
                setValue((current) => ({ ...current, [option.key]: checked }));
              }}
              className="h-6 w-6 shrink-0 accent-lime-300"
            />
          </label>
        ))}
      </fieldset>
      <p className="text-sm leading-6 text-slate-400">
        These choices filter your in-app bell and Notification Centre. Essential security and maintenance notices remain visible.
        Muting an update does not delete the activity, correction history or earned badge. No email or background push subscription is created here.
      </p>
      <button disabled={busy} className="min-h-11 rounded-xl bg-lime-300 px-5 py-3 font-black text-slate-950 disabled:opacity-40">
        {busy ? 'Saving…' : 'Save notification preferences'}
      </button>
      <p role="status" aria-live="polite" className="text-sm text-sky-200">{message}</p>
    </form>
  );
}
