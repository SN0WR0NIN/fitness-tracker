...
test('admins can vet proof, distance and pace and persist or clear score overrides', async ({ browser, baseURL }) => {
  ...
  try {
    ...
    const memberContext = await login(browser, baseURL, member, password); contexts.push(memberContext);
    const adminContext = await login(browser, baseURL, admin, password); contexts.push(adminContext);
    ...
    let saved = await db.activity.findUniqueOrThrow({ where: { id: activity.id }, include: { pointsLog: true } });
    ...
    await json(await adminContext.request.patch(`/api/admin/activities/${activity.id}`, { data: { basePointsOverride: null, totalPointsOverride: null } }));
    saved = await db.activity.findUniqueOrThrow({ where: { id: activity.id }, include: { pointsLog: true } });
