...
type AchievementRow={name:string;description:string;category:ProfileAchievement['category'];tier:ProfileAchievement['tier'];target:number;unit:string;value:number;};
export async function getEngineAchievements(userId:string):Promise<ProfileAchievement[]>{
  const rows=await prisma.$queryRaw<AchievementRow[]>`SELECT d.name,d.description,d.category,d.tier,d.target,d.unit,
  ...
  return rows.map((row)=>({name:row.name,description:row.description,category:row.category,tier:row.tier,unlocked:row.value>=row.target,progress:Math.max(0,Math.min(row.value/row.target,1)),progressLabel:`${row.unit==='pts'?Math.min(row.value,row.target).toFixed(1):Math.min(row.value,row.target)} / ${row.target} ${row.unit}`}));
}
