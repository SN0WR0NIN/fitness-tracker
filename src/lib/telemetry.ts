  ...
  return {
    ...
    failure(error: unknown, extra: Record<string, unknown> = {}) {
      ...
      try {
          ...
          try {
            await prisma.performanceMetric.create({
              data: {
                ...
                value: 1,
                release: releaseId(),
                metadata: metricMetadata({ durationMs, ...extra }),
              },
            });
          } catch (storeError) {
            console.error(
              JSON.stringify({
                level: "warning",
                event: "request_failure_store_failed",
                route,
                error:
        ...
        // Logging still reaches the runtime when after() is unavailable.
      }
    },
  };
}
...
export function performanceLog(
  ...
  if (!shouldPersistPerformance()) return;
  const route =
    typeof extra.route === "string" ? extra.route.slice(0, 160) : "unknown";
