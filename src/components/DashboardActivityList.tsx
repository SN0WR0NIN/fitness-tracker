...
export default function DashboardActivityList({ activities, users, profileId, summary, shown }: { activities: DashboardActivity[]; users: SelectableUser[]; profileId: string; summary: ActivitySummary; shown: number }) {
  ...
  useEffect(() => {
    setRows(activities);
    setStats(summary);
  }, [activities, summary.approved, summary.pending, summary.total]);
  ...
  const nextShown = Math.min(stats.total, Math.max(shown, rows.length) + 8);

  return <details id="my-activities" open className="dashboard-fold"><summary>My activities</summary><section className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04]">
    ...
    {rows.length < stats.total ? <div className="border-t border-white/5 p-4 text-center"><Link href={`/dashboard?activities=${nextShown}#my-activities`} className="inline-flex rounded-xl border border-white/10 bg-white/5 px-5 py-2.5 text-sm font-black text-slate-200 transition hover:bg-white/10">Load more · {stats.total - rows.length} remaining</Link></div> : null}
  </section></details>;
}
