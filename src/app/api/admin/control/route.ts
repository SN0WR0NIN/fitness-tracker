...
const SettingsSchema = z.object({ challengeName: z.string().trim().min(3).max(100), startDate: z.coerce.date(), endDate: z.coerce.date(), weeklyGoal: z.number().positive().max(10000), scoringRules: ScoringRulesSchema }).refine((settings) => settings.endDate > settings.startDate, { message: 'End date must be after start date' });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === 'settings.update') {
      ...
      await prisma.$executeRawUnsafe('UPDATE "ChallengeSetting" SET "challengeName"=$1,"startDate"=$2,"endDate"=$3,"weeklyGoal"=$4,"scoringRules"=$5::jsonb,"updatedAt"=CURRENT_TIMESTAMP WHERE "id"=$6', data.challengeName, data.startDate, data.endDate, data.weeklyGoal, JSON.stringify(data.scoringRules), 'primary');
      await recordAdminAudit(guard.userId, 'Updated challenge settings', data.challengeName);
    ...
    console.error('Admin control action failed:', error);
