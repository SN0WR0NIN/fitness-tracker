...
let core;
try {
  const filename = path.join(temporary, 'core.json');
  fs.writeFileSync(filename, JSON.stringify({ ...backup, version: 6 }), { mode: 0o600 });
  core = spawnSync(process.execPath, [validator, filename], { encoding: 'utf8' });
...
for (const log of Array.isArray(backup.pointsLogs) ? backup.pointsLogs : []) {
  ...
  check(log.totalPoints === activities.get(log.activityId)?.points, 'PointsLog total differs from activity.');
  check(Number.isFinite(Date.parse(log.createdAt)), 'PointsLog creation time is missing.');
}
