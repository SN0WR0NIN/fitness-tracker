...
export default function WeeklyRecapCard({name,columnName,rank,participantCount,weeklyScores,goalHistory,activities}:Props){
  ...
  },[activities,goalHistory,weeklyScores]);
  ...
  const buildImage=async()=>{
    ...
    ctx.fillStyle='#94a3b8';ctx.font='500 26px Arial, sans-serif';ctx.fillText(`${columnName??'No column'} · ${recap.dateRange}`,72,282);
    ...
    const rankText=rank?`Season rank #${rank} of ${participantCount}`:`Season rank pending · ${participantCount} participants`;
  ...
  return <details open className="dashboard-fold"><summary>Weekly recap card</summary><section className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 sm:p-6">
    <div className="overflow-hidden rounded-3xl border border-lime-300/15 bg-[radial-gradient(circle_at_top_right,_rgba(180,255,69,0.16),_transparent_34%),radial-gradient(circle_at_bottom_left,_rgba(251,146,60,0.12),_transparent_36%),#0f172a] p-6 sm:p-8">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-lime-300"><Sparkles className="h-4 w-4" />Week {recap.weekNumber} recap</p><h2 className="mt-3 text-3xl font-black sm:text-4xl">{name}</h2><p className="mt-1 text-sm text-slate-400">{columnName??'No column'} · {recap.dateRange}</p></div><div className="text-right"><p className="text-5xl font-black text-lime-300">{recap.score.toFixed(1)}</p><p className="text-xs font-bold uppercase tracking-wide text-slate-500">weekly points</p></div></div>
