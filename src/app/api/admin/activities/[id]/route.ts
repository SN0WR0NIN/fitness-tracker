...
const halfPoint = z
  ...
  .refine(
    ...
    "Total points must use 0.5-point increments",
  );
...
const EditActivitySchema = z
  .object({
    category: z
      .enum(["RUN", "CYCLE", "SWIM", "WALK_OR_HIKE", "TROOP_GAMES"])
      .optional(),
    distance: z
      .number()
    ...
    pace: z
      .number()
      .positive("Pace must be greater than zero")
      .max(60)
      .nullable()
      .optional(),
    ...
    basePointsOverride: z
      .number()
      .nonnegative("Base points cannot be negative")
      .max(100000)
      .nullable()
      .optional(),
  ...
  .refine(
    (value) =>
      !value.proofUrls ||
      new Set(value.proofUrls).size === value.proofUrls.length,
    {
      message: "The same proof photo cannot be attached twice.",
      path: ["proofUrls"],
    },
  );
...
export async function PATCH(
  ...
  try {
    ...
    const { reviewNote, ...activityChanges } = data;
    ...
    if (data.companionUserId) {
      ...
      const companion = await prisma.user.findUnique({
        where: { id: data.companionUserId },
      });
      if (!companion) {
        return NextResponse.json(
          { error: "Selected companion was not found" },
          { status: 400 },
        );
      }
    }

    const activity = Object.keys(activityChanges).length
      ? await updateActivity(id, activityChanges)
    ...
    const scoreOverrideChanged =
      data.basePointsOverride !== undefined ||
      data.totalPointsOverride !== undefined;
    const action = scoreOverrideChanged
      ? "Overrode activity score"
      : reviewNote !== undefined
        ? "Reviewed activity score"
        : "Corrected activity";
    await recordAdminAudit(guard.userId, action, id, {
      ...activityChanges,
      ...(reviewNote !== undefined ? { reviewNote } : {}),
    });
    return NextResponse.json({ ...activity, reviewNote: reviewNote ?? "" });
  } catch (error) {
    if (error instanceof ActivityEditError)
      return NextResponse.json(
        { error: error.message },
        { status: error.status },
      );
    if (error instanceof ZodError) {
      return NextResponse.json(
        { error: error.issues[0]?.message || "Invalid activity details" },
        { status: 400 },
      );
    }
