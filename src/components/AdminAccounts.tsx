...
type Account = { id: string; name: string; email: string; emailConfirmedAt: string | null; canResetPassword: boolean; columnId: string | null; username: string | null; pendingEmail: string | null; mustChangePassword: boolean; canProvision: boolean; role: string; unclaimed: boolean; temporaryPasswordExpiresAt: string | null };
export default function AdminAccounts({ users, columns }: { users: Account[]; columns: { id: string; name: string }[] }) {
  ...
  async function sendPasswordReset(user: Account) {
    if (!window.confirm(`Send a one-time password reset link to ${user.email}?`)) return;
    setResetBusyId(user.id); setMessage('');
    try {
      const response = await fetch('/api/admin/password-resets', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user.id }) });
      const data = await response.json(); if (!response.ok) throw new Error(data.error);
      setMessage(`Password reset email sent to ${user.email}.`); router.refresh();
  ...
  return <main className="mx-auto min-h-screen max-w-3xl space-y-6 px-4 py-8 text-white"><Link href="/admin/users" className="text-orange-300">← Manage users</Link><h1 className="text-3xl font-bold">Participant accounts</h1><p className="text-slate-300">Activate imported records, confirm participant emails, or send a secure one-time password reset link.</p>{message ? <p role="status" className="rounded-xl border border-white/10 bg-white/5 p-4">{message}</p> : null}
