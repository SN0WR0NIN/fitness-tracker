...
import { BookOpen, Gauge, History, Home, LogOut, Menu, Plus, ShieldCheck, UserRound, Users, X } from 'lucide-react';
      ...
      <header className="sticky top-0 z-40 border-b border-white/10 bg-slate-950/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6">
          ...
          <div className="hidden items-center gap-1 md:flex">
            ...
            <Link href="/admin" className={`flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold ${pathname === '/admin' ? 'bg-white/10 text-amber-300' : 'text-slate-400 hover:text-white'}`}><ShieldCheck className="h-4 w-4" />Control</Link>
            <Link href="/admin/officers" className={`flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold ${pathname.startsWith('/admin/officers') ? 'bg-white/10 text-amber-300' : 'text-slate-400 hover:text-white'}`}><Users className="h-4 w-4" />Officers</Link>
        ...
        {open ? <div className="border-t border-white/10 p-3 md:hidden"><div className="mx-auto grid max-w-7xl gap-1">{nav.map(({ href, label, icon: Icon }) => <Link key={href} href={href} onClick={() => setOpen(false)} className="flex items-center gap-3 rounded-xl px-4 py-3 font-bold text-slate-200 hover:bg-white/5"><Icon className="h-4 w-4" />{label}</Link>)}<Link href="/admin" onClick={() => setOpen(false)} className="flex items-center gap-3 rounded-xl px-4 py-3 font-bold text-amber-200 hover:bg-white/5"><ShieldCheck className="h-4 w-4" />Control centre</Link><Link href="/admin/officers" onClick={() => setOpen(false)} className="flex items-center gap-3 rounded-xl px-4 py-3 font-bold text-amber-200 hover:bg-white/5"><Users className="h-4 w-4" />Manage officers</Link><button onClick={signOut} className="flex items-center gap-3 rounded-xl px-4 py-3 text-left font-bold text-rose-300"><LogOut className="h-4 w-4" />Log out</button></div></div> : null}
      </header>
      {children}
