...
const SettingsSchema = z.object({ challengeName: z.string().trim().min(3).max(100), startDate: z.coerce.date(), endDate: z.coerce.date(), weeklyGoal: z.number().positive().max(10000), scoringRules: ScoringRulesSchema }).strict().refine((settings) => settings.endDate > settings.startDate, { message: 'End date must be after start date' });
  ...
  try {
    ...
    if (action === 'settings.update') {
      ...
      const changed = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
        ...
        await tx.$executeRaw`INSERT INTO "AdminAudit" (id,"actorId","actorName",action,target,details) VALUES (${randomUUID()},${guard.userId},${actor?.name ?? 'Administrator'},'Updated challenge settings',${data.challengeName},${JSON.stringify(data)}::jsonb)`;
        return true;
      });
      if (!changed) return NextResponse.json({ error: 'Settings are unavailable or read-only. Unlock in Maintenance controls before changing rules.' }, { status: 423 });
